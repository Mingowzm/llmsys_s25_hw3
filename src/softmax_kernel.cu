...
__global__ void ker_attn_softmax(T *inp, const T *attn_mask, int from_len,
  ...
  for (int token_id = blockIdx.x * token_per_reduce; token_id < from_len;
    ...
    for (int i = 0; i < token_per_reduce && (token_id + i) < from_len; i++) {
      for (int j = 0; j < ele_per_thread; j++) {
        inp_val[i][j] = static_cast<T>(val[i][j] * s_sum[i]);
      }
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  size_t gsize = rows * softmax_len * sizeof(float);

  cudaMalloc(&d_out_grad, gsize);
  cudaMalloc(&d_soft_inp, gsize);
  cudaMemcpy(d_soft_inp, soft_inp, gsize, cudaMemcpyHostToDevice);
  ...
  if (softmax_len <= 32) {
    ...
    throw std::runtime_error("Sequence length greater than 2048");
  }

  // Copy results back to host
  cudaMemcpy(out_grad, d_out_grad, gsize, cudaMemcpyDeviceToHost);
